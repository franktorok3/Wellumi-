...
function readNumeric(value) {
  return roundValue(Number(value));
}
...
function formatNutritionEntries(nutritionData) {
  ...
  for (const [rawKey, rawValue] of Object.entries(per100g)) {
    ...
    if (!canonicalValues.has(canonicalKey)) canonicalValues.set(canonicalKey, value);
  }
  ...
  return { basis: entries.length ? 'per 100 g' : null, entries };
}
